...
#ifdef CONFIG_S5PV210_GARNETT_DELTA
static finger_status_t fingerStatus[MAX_USING_FINGER_NUM]={0};
...
void qt_Multitouchscreen_Init(void)
{
    ...
    touchscreen_config.xrange   = DIGITIZER_HEIGHT;
    touchscreen_config.yrange   = DIGITIZER_WIDTH;
...
#ifdef CONFIG_S5PV210_GARNETT_DELTA
static void _input_report_finger_info(struct i2c_ts_driver * pTsDriver, report_finger_info_t * pFingerInfo, finger_status_t * pFingerStatus )
{
    unsigned long x, y;
    int action, area;
    ...
    switch ( area ) {
        case 1: // event from main screen
          ...
          input_report_abs(qt602240->input_dev, ABS_MT_WIDTH_MAJOR, pFingerInfo->size_id);
          input_mt_sync(qt602240->input_dev);
        ...
        case 3: // event from ticker screen
          input_report_abs(qt602240->input_dev2, ABS_MT_POSITION_X, x);
          input_report_abs(qt602240->input_dev2, ABS_MT_POSITION_Y, y);
          input_report_abs(qt602240->input_dev2, ABS_MT_TOUCH_MAJOR, status);
          input_report_abs(qt602240->input_dev2, ABS_MT_WIDTH_MAJOR, pFingerInfo->size_id);
          input_mt_sync(qt602240->input_dev2);
	...
	input_report_abs(pTsDriver->input_dev, ABS_MT_POSITION_X, pFingerInfo->x);
	input_report_abs(pTsDriver->input_dev, ABS_MT_POSITION_Y, pFingerInfo->y);
	input_report_abs(pTsDriver->input_dev, ABS_MT_TOUCH_MAJOR, pFingerInfo->pressure);	// 0見 Release, 틈玖 Press (Down or Move)
	input_report_abs(pTsDriver->input_dev, ABS_MT_WIDTH_MAJOR, pFingerInfo->size_id);	// (ID<<8) | Size
	input_mt_sync(pTsDriver->input_dev);
    
#endif /*CONFIG_MACH_TICKERTAPE*/
// SERI-END
}
#endif
			...
			if( quantum_msg[0] == 14 )
			{
				if((quantum_msg[1]&0x01) == 0x00)
				{
					...
					for ( i= 0; i<MAX_USING_FINGER_NUM; ++i )
					{
						...
						fingerInfo[i].pressure= 0;
						#ifdef CONFIG_S5PV210_GARNETT_DELTA
                        _input_report_finger_info( qt602240, &fingerInfo[i], &fingerStatus[i]);
                        #else 
						input_report_abs(qt602240->input_dev, ABS_MT_POSITION_X, fingerInfo[i].x);
						input_report_abs(qt602240->input_dev, ABS_MT_POSITION_Y, fingerInfo[i].y);
						input_report_abs(qt602240->input_dev, ABS_MT_TOUCH_MAJOR, fingerInfo[i].pressure);	// 0이면 Release, 아니면 Press 상태(Down or Move)
					...
					input_sync(qt602240->input_dev);
					#ifdef CONFIG_MACH_TICKERTAPE
					input_sync(qt602240->input_dev2);
				    #endif 
				}
				else
				{
...
//		  printk(";\n[TSP]Reset Chip!!!\n");
	...
	if(press == 1)
	{
		...
		if (y<MAIN_SCREEN_HEIGHT) {
			...
			input_report_abs(qt602240->input_dev2, ABS_X, x);
			//ZA: fix for Y coordinate being sent without reducing it for ticker area
			//input_report_abs(qt602240->input_dev2, ABS_Y, y);
			input_report_abs(qt602240->input_dev2, ABS_Y, y-(MAIN_SCREEN_HEIGHT+SOFTKEYS_SCREEN_HEIGHT));
			//ZA: end
			if( btn_report == 1)
				input_report_key(qt602240->input_dev2, BTN_TOUCH, 1);

			input_sync(qt602240->input_dev2);
			qt602240->screen_id = TICKER_SCREEN
		}
        #else /*CONFIG_MACH_TICKERTAPE*/
		input_report_abs(qt602240->input_dev, ABS_X, x);
		input_report_abs(qt602240->input_dev, ABS_Y, y);
		if( btn_report == 1)
			input_report_key(qt602240->input_dev, BTN_TOUCH, 1);
...
char qt_init = 1;
char t_key_init =0;
static int s5p_dsim_sysfs_set_qt_init_state(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
		return sprintf(buf, "%d\n", qt_init);
}

static DEVICE_ATTR(set_qt_init_state, 0666,
		s5p_dsim_sysfs_set_qt_init_state, NULL);

static int s5p_dsim_sysfs_set_touch_key_init(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
		return sprintf(buf, "%d\n", t_key_init);
}

static DEVICE_ATTR(set_touch_key_init, 0666,
		s5p_dsim_sysfs_set_touch_key_init, NULL);
#endif
...
//	DEBUG;
	...
	#ifdef CONFIG_MACH_TICKERTAPE
	// set bit for google soft keys buttons
...
#ifdef CONFIG_MACH_TICKERTAPE
	qt602240->input_dev2 = input_allocate_device();
	if (qt602240->input_dev2 == NULL) {
		ret = -ENOMEM;
		printk(KERN_ERR "qt602240_probe: Failed to allocate input device number 2\n");
		goto err_input_dev_alloc_failed;
	}
	qt602240->input_dev2->name = "qt602240_ts_input_tt";
	set_bit(EV_SYN, qt602240->input_dev2->evbit);
	set_bit(EV_KEY, qt602240->input_dev2->evbit);
	set_bit(BTN_TOUCH, qt602240->input_dev2->keybit);
	set_bit(EV_ABS, qt602240->input_dev2->evbit);
	input_set_abs_params(qt602240->input_dev2, ABS_X, 0, FULL_SCREEN_WIDTH, 0, 0);
	input_set_abs_params(qt602240->input_dev2, ABS_Y, 0, TICKER_SCREEN_HEIGHT, 0, 0);
#ifdef _SUPPORT_MULTITOUCH_
	input_set_abs_params(qt602240->input_dev2, ABS_MT_POSITION_X, 0, FULL_SCREEN_WIDTH, 0, 0);
	input_set_abs_params(qt602240->input_dev2, ABS_MT_POSITION_Y, 0, TICKER_SCREEN_HEIGHT, 0, 0);
	input_set_abs_params(qt602240->input_dev2, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(qt602240->input_dev2, ABS_MT_WIDTH_MAJOR, 0, 15, 0, 0);
#endif
	input_set_abs_params(qt602240->input_dev2, ABS_PRESSURE, 0, 255, 0, 0);
	input_set_abs_params(qt602240->input_dev2, ABS_TOOL_WIDTH, 0, 15, 0, 0);

	ret = input_register_device(qt602240->input_dev2);
	if (ret) {
...
err_input_register_device_failed:
    ...
    #ifdef CONFIG_MACH_TICKERTAPE
	input_free_device(qt602240->input_dev2);
    #endif
