...
static int keypad_scan(void)
{
	...
	#ifdef CONFIG_S5PV210_GARNETT_DELTA    // Added for Atlas Froyo
	for (col=0; col < KEYPAD_COLUMNS; col++) {
		...
		rval = ~(readl(key_base+S3C_KEYIFROW)) & 0x06 ; //((1<<KEYPAD_ROWS)-1) ;
		keymask[col] = rval; 
	}
	#else
    //for (col=0; col < KEYPAD_COLUMNS; col++) { /*Added for Atlas Froyo */
    col = 1;
    cval = KEYCOL_DMASK & ~((1 << col) | (1 << col+ 8)); // clear that column number and 
