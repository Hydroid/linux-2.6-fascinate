...
extern char lcd_wakeup_source_reg;
#endif
...
static int s3c_pm_enter(suspend_state_t state)
{
	...
	*/
	...
	{
		if( lcd_wakeup_source_reg	==  0 )
 			__raw_writel(0xFFFD, S5P_WAKEUP_MASK); //Wakeup - RTC_ALARM. Disabling KEY Interrupt as Wakeup reason.	
 		else if( lcd_wakeup_source_reg	==  1 )
 			__raw_writel(0xFFDD, S5P_WAKEUP_MASK); //Wakeup - RTC_ALARM. enabling KEY Interrupt as Wakeup reason.				
	}
#else
